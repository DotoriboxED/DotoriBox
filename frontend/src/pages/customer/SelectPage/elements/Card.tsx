...
const Filter = styled.div`
  ...
  width: calc(100% - 2em - 0.4rem);
  height: 16.1rem;
  position: absolute;
`

const Card = (props: any) => {
    const { setSelect, select, sample, amount, sampleInfo } = props;
    const { sampleName, price } = sample;
    ...
    return (
        <Each style={{borderColor: background}}>
            ...
            <ImageForm>
                {
                    amount > 0 ? <Image src={API_URL + '/sample/' + props.id + '/image'}/>
                        : <SoldOutImage src={API_URL + '/sample/' + props.id + '/image'}/>
                }
            ...
            <InvalidPopup isValid={sampleExists} setValid={setSampleExists} message={
                <div>매진된 상품입니다.<br/>아쉽지만 다른 샘플을 골라 주세요!</div>}/>
            <InvalidPopup isValid={infoState} setValid={setInfoState}
                          message={
                              <div>
